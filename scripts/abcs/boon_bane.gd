...
@export var _trigger: Constants.TRIGGER  ## the thing that causes the boonbane to apply
...
func _ready() -> void:
	...
	if _trigger == Constants.TRIGGER.on_interval and _interval_length == 0:
	...
	if _trigger == Constants.TRIGGER.on_interval:
