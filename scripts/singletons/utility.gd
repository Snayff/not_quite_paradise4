...
#region FUNCS
## set the collision layers and masks, based on team and targeting, to the relevant "_collision" collision layers.
func update_body_collisions(node: CollisionObject2D, team: Constants.TEAM, target_option: Constants.TARGET_OPTION, target_actor: CombatActor = null) -> void:
	_update_collisions(node, team, target_option, Constants.COLLISION_LAYER.team1_collision, Constants.COLLISION_LAYER.team2_collision, target_actor)

## set the collision layers and masks, based on team and targeting, to the relevant "hurtbox" collision layers.
func update_hitbox_hurtbox_collision(node: CollisionObject2D, team: Constants.TEAM, target_option: Constants.TARGET_OPTION, target_actor: CombatActor = null) -> void:
	_update_collisions(node, team, target_option, Constants.COLLISION_LAYER.team1_hurtbox, Constants.COLLISION_LAYER.team2_hurtbox, target_actor)
...
func _update_collisions(
	...
	# LAYERS
	node.set_collision_layer_value(Constants.COLLISION_LAYER_MAP[Constants.COLLISION_LAYER.team1_collision], team == Constants.TEAM.team1)
	node.set_collision_layer_value(Constants.COLLISION_LAYER_MAP[Constants.COLLISION_LAYER.team2_collision], team == Constants.TEAM.team2)
...
#
		## LAYERS
		#node.set_collision_layer_value(Constants.COLLISION_LAYER_MAP[Constants.COLLISION_LAYER.team1_hurtbox], team == Constants.TEAM.team1)
		#node.set_collision_layer_value(Constants.COLLISION_LAYER_MAP[Constants.COLLISION_LAYER.team2_hurtbox], team == Constants.TEAM.team2)
#
		## MASKS
		## only same team
		#if target_option in [Constants.TARGET_OPTION.self_, Constants.TARGET_OPTION.ally]:
